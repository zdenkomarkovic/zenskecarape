  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-900">Filteri</h2>
        {hasActiveFilters && (
          <Button
            variant="ghost"
            size="sm"
      ...
      <Accordion type="multiple" className="w-full">
        {filters.sizes.length > 0 && (
          <AccordionItem value="size">
            <AccordionTrigger className="text-sm font-medium">
              Veličina {selectedFilters.sizes.length > 0 && `(${selectedFilters.sizes.length})`}
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-2">
                {filters.sizes.map((size) => (
                  <label key={size._id} className="flex cursor-pointer items-center gap-2">
                    <input
                      type="checkbox"
        ...
        {filters.colors.length > 0 && (
          <AccordionItem value="color">
            <AccordionTrigger className="text-sm font-medium">
              Boja {selectedFilters.colors.length > 0 && `(${selectedFilters.colors.length})`}
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-2">
                {filters.colors.map((color) => (
                  <label key={color._id} className="flex cursor-pointer items-center gap-2">
                    <input
                      type="checkbox"
        ...
        {filters.deniers.length > 0 && (
          <AccordionItem value="denier">
            <AccordionTrigger className="text-sm font-medium">
              Denier {selectedFilters.deniers.length > 0 && `(${selectedFilters.deniers.length})`}
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-2">
                {filters.deniers.map((denier) => (
                  <label key={denier._id} className="flex cursor-pointer items-center gap-2">
                    <input
                      type="checkbox"
                      ...
                      className="h-4 w-4 rounded border-gray-300 text-red-500 focus:ring-red-500"
                    />
                    <span className="text-sm text-gray-700">{denier.value}</span>
                  </label>
                ))}
              </div>
            </AccordionContent>
        ...
        {filters.productTypes.length > 0 && (
          <AccordionItem value="productType">
            <AccordionTrigger className="text-sm font-medium">
              Tip proizvoda {selectedFilters.productTypes.length > 0 && `(${selectedFilters.productTypes.length})`}
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-2">
                {filters.productTypes.map((type) => (
                  <label key={type.value} className="flex cursor-pointer items-center gap-2">
                    <input
                      type="checkbox"
                      checked={selectedFilters.productTypes.includes(type.value)}
                      onChange={() => handleFilterToggle('productTypes', type.value)}
                      className="h-4 w-4 rounded border-gray-300 text-red-500 focus:ring-red-500"
                    />
        ...
        {filters.features.length > 0 && (
          <AccordionItem value="features">
            <AccordionTrigger className="text-sm font-medium">
              Osobine {selectedFilters.features.length > 0 && `(${selectedFilters.features.length})`}
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-2">
